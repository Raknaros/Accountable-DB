...
CREATE TABLE acc.entities (entity_id INTEGER PRIMARY KEY UNIQUE,nombre_razon VARCHAR UNIQUE, tipo_documento VARCHAR REFERENCES TS02(tipo_documento) ON DELETE SET NULL,numero_documento VARCHAR UNIQUE,usuario_sol VARCHAR,clave_sol VARCHAR,activo BOOLEAN,observaciones VARCHAR);
---PENDING CREATE TABLE acc.entinfo(numero_documento VARCHAR UNIQUE,telefono INTEGER,email VARCHAR,domicilio_fiscal VARCHAR,distrito TEXT,ciudad TEXT,departamento TEXT,codigo_postal INTEGER)
...
CREATE TABLE acc.f0710(casilla smallint PRIMARY KEY, concepto VARCHAR);
