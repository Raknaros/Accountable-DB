...
END;
$$ LANGUAGE plpgsql;
--Asignar numero de operación y hora de operación
SELECT 
...
DECLARE
	...
	total DEC(10,2):= (CASE WHEN (NEW.subdiario = 5 AND NEW.destino <> 2) OR (NEW.subdiario = 8 AND NEW.destino <> 4) THEN NEW.igv*NEW.valor ELSE NEW.valor END) + NEW.isc + NEW.icbp + NEW.otros_cargos;
...
--Trigger/Preliquidacion general Pendiente Ventas Netas Gravadas Ley N° 31556 10% (154) Compras Netas Destinadas a vtas. gravadas Ley N° 31556 10% (156) 
